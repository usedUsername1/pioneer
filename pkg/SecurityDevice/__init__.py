...
class SecurityDevice:
    ...
    def get_object_info_from_device_conn(self, object_type, ObjectContainer):
        ...
        #TODO: sometimes, policies in the exact same container can have the same names (this is most likely a bug)
        # if that happens, then too bad. only the first policy encoutered will be processed and a fail insert message
        # will be generated for the next one.
        ...
        if objects_info is not None:
            if 'group' not in object_type:
                ...
                for GroupObject in group_objects:
                    if object_type == 'security_policy_group':
                        print(f"Creating relationships for {GroupObject._name}, UID {GroupObject._uid} IN CONAINER {GroupObject._PolicyContainer._name}")
